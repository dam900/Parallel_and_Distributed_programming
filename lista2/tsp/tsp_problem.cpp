...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        if (DEBUG)
            pretty_print(adj_mat);
        if (DEBUG)
            pretty_print(pheromones, true);
        if (DEBUG)
            std::cout << "Cost: " << std::setprecision(10) << cost(adj_mat, pheromones, {0, 1, 0}) << std::endl;
    }
    ...
    while (iter <= MAX_ITER) {
        int start = dist(gen);
        auto path = generate_path(start, n, adj_mat);
        double path_cost = cost(adj_mat, pheromones, path);
